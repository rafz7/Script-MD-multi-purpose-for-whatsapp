...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  let videoBuffer = fs.existsSync(videoPath)
    ...
    : null;
  let audioBuffer = fs.existsSync(audioPath)
  ? fs.readFileSync(audioPath)
  : null;
console.log("[MENU AUDIO PATH]", audioPath);
console.log("[MENU AUDIO EXISTS]", fs.existsSync(audioPath));
console.log("[MENU AUDIO BUFFER]", audioBuffer ? audioBuffer.length : "NULL");

  // Convert mp3 → ogg/opus agar diterima WhatsApp
  if (audioBuffer) {
    try {
      const { execSync } = await import("child_process");
      const tmpIn = path.join(process.cwd(), "_menu_audio_in.mp3");
      const tmpOut = path.join(process.cwd(), "_menu_audio_out.ogg");
      fs.writeFileSync(tmpIn, audioBuffer);
      execSync(`ffmpeg -y -i "${tmpIn}" -c:a libopus -b:a 128k -ar 48000 "${tmpOut}"`, { timeout: 15000 });
      audioBuffer = fs.readFileSync(tmpOut);
      try { fs.unlinkSync(tmpIn); fs.unlinkSync(tmpOut); } catch {}
    } catch {
      // ffmpeg tidak ada, tetap pakai mp3 asli
    }
  }
...
Kamu bisa pakai aku buat cari info, ambil data, atau bantu hal-hal sederhana langsung lewat WhatsApp — praktis tanpa ribet.\n\n`;
  ...
  try {
    ...
    if (imageBuffer) {
      try {
        const resized = await (await getSharp())(imageBuffer)
          .resize(300, 300, { fit: "cover" })
          .jpeg({ quality: 80 })
          .toBuffer();
    ...
    console.error("[Menu V9] Error:", v9Error);
    await sendFallback(
      m,
      sock,
      text,
  ...
  // AUDIO KHUSUS V9 — dijalankan selalu, di luar try-catch utama
  if (audioBuffer) {
    await sock.sendMessage(m.chat, {
      audio: audioBuffer,
      mimetype: "audio/ogg; codecs=opus",
      ptt: true,
    }).catch(e => console.error("[Menu V9] Audio error:", e.message));
  }
  break;
}
...
╰─────────────`,
      ...
      case 15:
        try {
          ...
          // Ambil foto profil user, fallback ke pp-kosong
          let pp;
          try {
            pp = Buffer.from(
              (
            ...
            pp = fs.existsSync("./assets/images/pp-kosong.jpg")
              ? fs.readFileSync("./assets/images/pp-kosong.jpg")
              : thumbBuffer || imageBuffer;
          }
          ...
          // Resize thumbnail untuk ftroliQuoted
          let ppThumb = null;
          try {
            ppThumb = await (await getSharp())(pp)
              .resize({ width: 300, height: 300 })
              .toBuffer();
          } catch {
            ppThumb = thumbBuffer || null;
          }
          ...
          // Ambil data cuaca BMKG, fallback kalau API down
          let titles = `🤖 ${botConfig.bot?.name || "Ourin-AI"}`;
          try {
            ...
            titles = `🌡️ ${suhu}°C | ${emojiCuaca} ${cuaca}`;
          } catch {
            console.warn("[Menu V15] BMKG API gagal, pakai fallback title");
          }

          // Resize jpegThumbnail
          let jpegThumb = thumbBuffer;
          try {
            const thumbSrc = fs.existsSync("./assets/images/ourin2.jpg")
              ? fs.readFileSync("./assets/images/ourin2.jpg")
              : thumbBuffer;
            if (thumbSrc) {
              jpegThumb = await (await getSharp())(thumbSrc)
                .resize({ width: 300, height: 300 })
                .toBuffer();
            }
          } catch {
            jpegThumb = thumbBuffer;
          }

          const footerText15 = `🌿 Halo *${m.pushName}* 👋
...
Silahkan tekan tombol dibawah untuk memilih category`;
          ...
          // Gunakan generateWAMessageFromContent + relayMessage (bukan sock.sendMessage langsung)
          const msg15 = generateWAMessageFromContent(
            m.chat,
            {
              viewOnceMessage: {
                message: {
                    ...
                    proto.Message.InteractiveMessage.fromObject({
                      ...
                      contextInfo: {
                        ...
                        externalAdReply: {
                          ...
                          previewType: "VIDEO",
                          thumbnail: imageBuffer || thumbBuffer,
            ...
            { userJid: m.sender, quoted: ftroliQuoted },
          );
          await sock.relayMessage(m.chat, msg15.message, {
            messageId: msg15.key.id,
          });
        } catch (v15Error) {
          console.error("[Menu V15] Error:", v15Error);
          await sendFallback(
            m,
            sock,
            text,
    ...
    // Case 9 sudah handle audio sendiri, skip agar tidak double
    if (audioEnabled && menuVariant !== 9 && audioBuffer) {
      await sock.sendMessage(m.chat, {
        audio: audioBuffer,
        mimetype: "audio/ogg; codecs=opus",
        ptt: true,
      }).catch(e => console.error("[Menu] Audio error:", e.message));
    }
  } catch (error) {
